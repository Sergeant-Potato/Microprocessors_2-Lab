 ...
 * CROSS2   ->      29
 * 
 * Moreover, the following pins are related to the one 7-Segment Displays
 * Seven Segement 1
 * A1 -> 30
 * B1 -> 31
 * C1 -> 32
...
const int sevenSeg1[] = {30, 31, 32, 33, 34, 35, 36};   // ADDED: Seven Segment 1 Pins
const int sevenSeg2[] = {37, 38, 39, 40, 41, 42, 43};   // ADDED: Seven Segment 2 Pins
...
void pedestrianBehavior(void){
  ...
  if(flag1 ==1){
    for(int i = 0; i<5;i++){
      sevenDisplay(1, 9-2*i);
      digitalWrite(trafficA[tSize-1],LOW);
      delay(1000);
      digitalWrite(trafficA[tSize-1],HIGH);
      sevenDisplay(1, 9-2*(i+1)+1);
      delay(1000);
    }
  }
  else{
    for(int i = 0; i<5;i++){
      sevenDisplay(2, 9-2*i);
      digitalWrite(trafficB[tSize-1],LOW);
      delay(1000);
      digitalWrite(trafficB[tSize-1],HIGH);
      sevenDisplay(2, 9-2*(i+1)+1);
      delay(1000);
    }
  }
  /*
...
void sevenDisplay(int m, int n){
  int tmp[7];
  if(m == 1){
    memcpy(tmp, sevenSeg1, sizeof tmp);
  }
  else{
    memcpy(tmp, sevenSeg2, sizeof tmp);
  }
  switch(n){
    case 0:
      digitalWrite(tmp[0], LOW);
      digitalWrite(tmp[1], LOW);
      digitalWrite(tmp[2], LOW);
      digitalWrite(tmp[3], LOW);
      digitalWrite(tmp[4], LOW);
      digitalWrite(tmp[5], LOW);
      digitalWrite(tmp[6], HIGH);
      break;
    case 1:
      digitalWrite(tmp[0], HIGH);
      digitalWrite(tmp[1], LOW);
      digitalWrite(tmp[2], LOW);
      digitalWrite(tmp[3], HIGH);
      digitalWrite(tmp[4], HIGH);
      digitalWrite(tmp[5], HIGH);
      digitalWrite(tmp[6], HIGH);
      break;
    case 2:
      digitalWrite(tmp[0], LOW);
      digitalWrite(tmp[1], LOW);
      digitalWrite(tmp[2], HIGH);
      digitalWrite(tmp[3], LOW);
      digitalWrite(tmp[4], LOW);
      digitalWrite(tmp[5], HIGH);
      digitalWrite(tmp[6], LOW);
      break;
    case 3:
      digitalWrite(tmp[0], LOW);
      digitalWrite(tmp[1], LOW);
